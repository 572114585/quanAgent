...
│   ├── whitelist.py             # _ShellWhitelistFilter（命令白名单）+ 组装 backend 单例
...
- [x] **多层安全沙箱**（位于 `sandbox/` 包）：
  - Shell 命令白名单（python/ls/cat 等只读探查 + Node 构建链）
  - Skill 脚本白名单（启动时 glob `skills/*/scripts/*.{py,sh}`，新增脚本需重启服务才生效）
...
| `tool_result` | 工具执行返回（callId、name、output、可选 `subagentId`），状态 completed |
