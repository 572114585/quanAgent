...
When the request mentions a specific product, brand, technology, SDK, or event you're not 100% sure about, the **first** action is `web_search` to verify existence, release status, latest version, and key specs from authoritative sources. Never assert from training data.
...
- [ ] **Step 0 ran** if any specific product/brand was named — facts verified via `web_search`, not assumed
...
- [ ] **Rendered to PNG for non-browser delivery channels** (see "Delivery to Non-Browser Channels" below)

---

## Delivery to Non-Browser Channels

**The HTML you produce must be viewable by the user.** In chat-based delivery channels (e.g., WeChat/IM, where the user cannot open a local `.html` file in a browser), a raw HTML file is unusable — the user receives a file they can only download, not view inline. You must also produce a screenshot so the design can be seen immediately.

### Mandatory final step for chat/IM channels

After the HTML is written to `output/`, call the `render_html` tool to screenshot it to PNG, then deliver **both** files:

```
render_html(html_path="output/landing-page.html")
```

Defaults render the full page at 1440×900 viewport with a `networkidle` wait + 1.5s buffer (enough for React/Chart.js/D3/oklch to finish). Override for special sizes:

| Output type | Call |
|---|---|
| Landing page / dashboard / website (default) | `render_html(html_path="output/x.html")` |
| 1920×1080 slide deck | `render_html(html_path="output/deck.html", width=1920, height=1080)` |
| iPhone prototype (full_page=False, capture viewport only) | `render_html(html_path="output/proto.html", width=390, height=844, full_page=False)` |

### Why both files

- **PNG** → rendered inline in the chat (user sees the design instantly, no download needed)
- **HTML** → delivered as a file (user can download and open in a browser for the **interactive** experience — animations, Tweaks panel, clickable prototype — that a static screenshot cannot convey)

Both go to `output/`; the delivery channel picks up both. Never deliver HTML alone on a chat/IM channel — the user would get an un-viewable file.

> Note: `render_html` uses a headless Chromium that fully executes React/Babel/Chart.js/D3 and renders modern CSS (oklch, container queries, backdrop-filter) faithfully. If rendering fails (e.g., CDN blocked), fall back to telling the user to open the HTML file in a browser directly.
...
**Path convention**: all `references/...` paths below are relative to this skill's directory. When calling `read_file`, prefix with the full skill path: `skills/web-design-engineer/references/...`. For example, `references/style-recipes/linear.md` → `read_file(file_path="skills/web-design-engineer/references/style-recipes/linear.md", limit=200)`.
