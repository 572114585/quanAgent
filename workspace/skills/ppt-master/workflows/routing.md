...
| Ordinary 10–12 page, 16:9 business-deck request without an explicit fidelity, preview, narration, animation, template, or manual-review requirement | Activate QuanAgent **Fast Generate**. It creates only `fast_contract.json` and invokes `ppt_fast_build(project_path)`; do not load Default or Quick. |
