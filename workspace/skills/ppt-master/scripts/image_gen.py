...
def _allowed_backends() -> frozenset[str]:
    """Return the host-approved image backends (Volcengine only by default)."""
    raw = os.environ.get("PPT_ALLOWED_IMAGE_BACKENDS", "volcengine")
    values = {value.strip().lower() for value in raw.split(",") if value.strip()}
    return frozenset(values or {"volcengine"})
...
BACKEND_REGISTRY = {
    ...
    "volcengine": {
        ...
        "key_hint": "AGENT_API_KEY (Agent Plan)",
...
def _resolve_backend() -> tuple[object, str]:
    ...
    if backend_name:
        ...
        if canonical not in allowed or canonical != "volcengine":
            print(
                "Error: this QuanAgent PPT Master integration permits only "
                "the Volcengine Seedream backend. "
                f"Requested: {canonical}; allowed: {', '.join(sorted(allowed))}.",
                file=sys.stderr,
            )
            sys.exit(1)
