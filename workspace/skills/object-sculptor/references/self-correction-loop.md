...
1. Capture a rendered screenshot with the DeepAgent **`render_html` tool** on `output/<name>.html` (`wait_ms≥2500`). Do not start `http.server` / other local servers, background shells, or a second browser stack.
