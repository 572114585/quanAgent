...
<template>
  ...
  >
    ...
    <div :class="['flex-1 min-w-0 max-w-[85%]', isUser ? 'flex flex-col items-end' : '']">
      ...
      <div
        v-if="!isUser && hasWebRefs && !isError"
        class="w-full mt-2"
      >
        <div class="rounded-md border border-border bg-surface-elevated/60 overflow-hidden">
          <button
            ...
            @click="toggleWebRefs"
          >
            <Globe class="size-3.5 shrink-0 text-ink-subtle" />
            <span class="text-xs text-ink-subtle">联网引用</span>
            <span class="text-[10px] text-ink-subtle/70 font-mono">({{ webReferences?.length }})</span>
            <ChevronRight
              class="size-3.5 text-ink-subtle ml-auto transition-transform duration-200"
              :class="{ 'rotate-90': webRefsExpanded }"
            />
          </button>
          <div
            v-show="webRefsExpanded"
            class="px-2 pb-2 pt-1 border-t border-border bg-surface-muted/30 space-y-1.5"
          >
            <div
              v-for="(ref, idx) in webReferences"
              :key="ref.url || idx"
              class="rounded border border-border/60 bg-surface-elevated px-2 py-1.5"
            >
              <div class="flex items-center gap-1.5 mb-1 min-w-0">
                <span class="text-[10px] font-mono text-ink-subtle/70 shrink-0">#{{ idx + 1 }}</span>
                <a
                  :href="ref.url"
                  target="_blank"
                  rel="noopener noreferrer"
                  class="text-[11px] text-ink truncate hover:underline min-w-0"
                  :title="ref.url"
                >{{ ref.title || hostname(ref.url) }}</a>
                <span class="text-[10px] text-ink-subtle/60 ml-auto shrink-0 font-mono">{{ hostname(ref.url) }}</span>
              </div>
              <pre
                v-if="ref.snippet"
                class="text-[11px] font-mono whitespace-pre-wrap break-all text-ink-subtle max-h-32 overflow-auto"
              >{{ previewText(ref.snippet) }}</pre>
            </div>
          </div>
        </div>
      </div>
